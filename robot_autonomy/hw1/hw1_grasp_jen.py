    ...
    def eval_grasp(self, grasp):
        with self.robot:
            ...
            try:
                ...
                # for each contact
                G = np.empty([6,1]) #the wrench matrix
                ...
                for c in contacts:
                    ...
                    dir_t = dir.T
                    G_curr = np.empty([6,1])
                    G_curr[0:6,0] = np.concatenate((dir,np.cross(pos,dir)), axis=0)
                    G = np.concatenate((G, G_curr), axis = 1)
                    #TODO use G to compute scores as discussed in class
                #print "This is G", G.shape
                det_G = np.linalg.det(np.dot(G,G.T))
                if det_G < 0:
                    Q = np.sqrt(det_G)
                    #print "Q is negative", Q
