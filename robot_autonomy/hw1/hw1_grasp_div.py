...
class RoboHandler:
  ...
  #displays the grasp
  def show_grasp(self, grasp, delay=4):
  ...
  def order_grasps_noisy(self):
    ...
    for i in range(0,6):
      print "Noisy"
      self.show_grasp(self.grasps_ordered_noisy[i])
      #self.show_grasp(self.grasps_ordered_noisy[-i])	
	
  
  def combo_grasps(self, ):

    self.grasps_ordered_combo = self.grasps.copy() #you should change the order of self.grasps_ordered
    for grasp in self.grasps_ordered_combo:
      grasp[self.graspindices.get('performance')] = self.eval_grasp(grasp)

    self.grasps_ordered_noisy_combo = self.grasps.copy() #you should change the order of self.grasps_ordered_noisy
    #TODO set the score with your evaluation function (over random samples) and sort
    for grasp_noise in self.grasps_ordered_noisy_combo:
      grasp_noise = self.sample_random_grasp(grasp_noise)
      grasp_noise[self.graspindices.get('performance')] = self.eval_grasp(grasp_noise)
      Q_combo[self.graspindeices.get('performance')] = grasp_noise[self.graspindices.get('performance')] + grasp[self.indices.get('performance')]

    order_combo = np.argsort(self.Q_combo[:,self.graspindices.get('performance')[0]])
    order_combo = order_combo[::-1]
    self.Q_combo = self.Q_combo[order_combo]

    for i in range(0,6):
      print "These are combo"
      self.show_grasp(self.Q_combo[i])
...
#            finestep              = None )
  ...
  def sample_random_grasp(self, grasp_in):
    ...
    RAND_DIST_SIGMA = 0.1 #TODO you may want to change this
    pos_orig = grasp[self.graspindices['igrasppos']]
    #TODO set a random position
    noise = np.random.normal(0,RAND_DIST_SIGMA,3)
    pos_mod = pos_orig + noise
    ...
    noise = np.random.normal(0,RAND_ANGLE_SIGMA,3)
    dir_mod = dir_orig + noise
    roll_mod = roll_orig + noise
    
    grasp[self.graspindices.get('igrasppos')] = pos_mod
    grasp[self.graspindices.get('igraspdir')] = dir_mod
    grasp[self.graspindices.get('igrasproll')] = roll_mod
