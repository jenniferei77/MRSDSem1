  ...
  def order_grasps_noisy(self):
    ...
    for grasp in self.grasps_ordered_noisy:
      ...
      for i in range(0,5):
        ...
        curr_worst = self.eval_grasp(grasp)
        if worst > curr_worst:
          worst = curr_worst
      grasp[self.graspindices.get('performance')] = worst
    # sort!
...
#            finestep              = None )
  ...
  def sample_random_grasp(self, grasp_in):
    ...
    RAND_DIST_SIGMA = np.random.random_sample()/10 #TODO you may want to change this
    ...
    RAND_ANGLE_SIGMA = (np.random.random_sample()/10) + 0.05 #TODO you may want to change this
