...
public:
    ...
    void setTagLocations(float x_det, float y_det, float z_det)
    {
        ...
        y_loc = y_det;
        y_loc = y_loc*(320) + 320;
        x_arr.push_back(x_loc+90);
        y_arr.push_back(y_loc-50);
    }
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        if (x_loc < cv_ptr->image.rows && y_loc < cv_ptr->image.cols){
            cv::circle(cv_ptr->image, cv::Point(x_loc + 90, y_loc - 50), 10, CV_RGB(0,255,0));
